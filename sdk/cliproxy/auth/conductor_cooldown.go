...
var minQuotaCooldownFloorOverride atomic.Int64

func getMinQuotaCooldownFloor() time.Duration {
	if v := minQuotaCooldownFloorOverride.Load(); v > 0 {
		return time.Duration(v)
	}
	return minQuotaCooldownFloor
}

// SetMinQuotaCooldownFloorForTest configures the minimum quota cooldown floor in tests.
func SetMinQuotaCooldownFloorForTest(d time.Duration) func() {
	prev := minQuotaCooldownFloorOverride.Load()
	minQuotaCooldownFloorOverride.Store(d.Nanoseconds())
	return func() {
		minQuotaCooldownFloorOverride.Store(prev)
	}
}
						...
						case 429:
							...
							if !disableCooling {
								if result.RetryAfter != nil {
									...
									if floor := getMinQuotaCooldownFloor(); cooldown < floor {
										cooldown = floor
									}
