...
func (m *Manager) Execute(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	if m.HomeEnabled() {
		...
		if isUnauthorizedError(errHome) {
			return resp, unwrapExecutionBoundaryError(errHome)
		}
		return resp, unwrapRequestStopError(wrapRouteExhaustion(errHome, tracker))
	}
...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	if lastErr != nil {
		...
		if errors.As(lastErr, &bootstrapErr) && bootstrapErr != nil {
			if m.HomeEnabled() {
				return streamErrorResult(bootstrapErr.Headers(), unwrapExecutionBoundaryError(lastErr)), nil
			}
			return streamErrorResult(bootstrapErr.Headers(), wrapRouteExhaustion(bootstrapErr.cause, tracker)), nil
		}
		if isUnauthorizedError(lastErr) {
			return nil, lastErr
		}
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int, extra ...any) (cliproxyexecutor.Response, error) {
	if len(providers) == 0 {
		return cliproxyexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	...
	if len(extra) > 0 {
		tracker, _ = extra[0].(*routeAttemptTracker)
	}
	if len(extra) > 1 {
		tried, _ = extra[1].(map[string]struct{})
	}
	if tried == nil {
...
func (m *Manager) executeCountMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int, extra ...any) (cliproxyexecutor.Response, error) {
	if len(providers) == 0 {
		return cliproxyexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel := authSelectionModelFromOptions(opts, req.Model)
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, homeRetryLimit *int, retryRound int, defaultRequestRetry int, extra ...any) (*cliproxyexecutor.StreamResult, error) {
	if len(providers) == 0 {
		return nil, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel := authSelectionModelFromOptions(opts, req.Model)
